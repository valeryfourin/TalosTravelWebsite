...
class TourController {
    async create(req, res, next) {
        try {
            const {title, country, description, img, startDate, endDate, nights, cost, type, activities} = req.body;
            // const {img} = req.files;
            // let fileName = uuid.v4() + ".jpg";
            // img.mv(path.resolve(__dirname, '..', 'static', filename));

            const tour = await Tour.create({title, country, description, img, startDate, endDate, nights, cost, type, activities});
